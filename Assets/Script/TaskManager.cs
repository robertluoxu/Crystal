...
public class TaskManager : NetworkBehaviour
{
    ...
    void Update() {
        TaskConnection.GetInstance().Update();
    }
    ...
    public override void OnStartLocalPlayer() {
        if (transform.tag == "Police") {
            socketId = "Police" + netId;
        }
        else if(transform.tag == "Engineer"){
            socketId = "Engineer" + netId;
        }
        else {
            socketId = "Drector" + netId;
        }
        TaskConnection.GetInstance().StartWebSocket(this.ReceiveMessage, socketId);
    }
    ...
    private void OnGUI() {
        ...
        if(taskInfos.Count > 0) {
            GUI.Box (new Rect (Screen.width - 260, 10, 250, 150), "任务");
            int index = 0;
            foreach (TaskInfo message in taskInfos.ToArray())
            {
                GUI.Label (new Rect (Screen.width - 245, 30+80*index, 250, 50), message.taskName+":"+message.taskDescription);
                if (GUI.Button(new Rect(Screen.width - 245, 30+80*index+40, 250, 50), "完成任务")){
                    TaskConnection.GetInstance().SendWebSocketMessage(message.taskId);
                    taskInfos.Remove(message);
                }
                index++;
            }
        }
    ...
    public Action<string> ReceiveMessage = (string item) => {
        try
        {
            TaskInfo taskInfo = JsonUtility.FromJson<TaskInfo>(item);
            // taskInfos.Clear();
